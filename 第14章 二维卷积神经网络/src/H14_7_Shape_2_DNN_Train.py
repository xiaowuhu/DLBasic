...
def build_dnn_model():
    model = Sequential()
    # 线性模型
    # model.add_op(layer.Linear(784, 2, init_method="kaiming", optimizer="Adam"))
    # model.set_classifier_loss_function(layer.SoftmaxCrossEntropy())
    # 非线性模型
    model.add_op(layer.Linear(784, 32, init_method="kaiming", optimizer="Adam"))
    model.add_op(layer.Relu())
    model.add_op(layer.Linear(32, 4, init_method="kaiming", optimizer="Adam"))
    model.set_classifier_loss_function(layer.SoftmaxCrossEntropy())
    return model
...
def train_model(data_loader, model, model_name, show_loss=True):
    start = time.time()
    params = HyperParameters(max_epoch=200, batch_size=64)
    lrs = LRScheduler.step_lrs(0.01, 0.9, 500)
    training_history = train(data_loader, model, params, lrs, model_name, checkpoint=1)
    end = time.time()
    print("用时:", end-start)
...
if __name__=="__main__":
    model_name = "Shape_conv_14_6_DNN"
    model = build_dnn_model()
    # data_loader = load_shape_data("train_shape_2.npz", "test_shape_2.npz", mode="vector")
    data_loader = load_shape_data("train_shape_4.npz", "test_shape_4.npz", mode="vector")
