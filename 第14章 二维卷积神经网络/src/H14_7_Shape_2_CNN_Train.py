...
def build_cnn_model(name):
    model = Sequential()
    if name == "1":
        # 一个卷积核
        c1 = layer.Conv2d((1,28,28), (1,3,3), stride=1, padding=0, optimizer="SGD")
    elif name == "2":
        # 两个卷积核
        c1 = layer.Conv2d((1,28,28), (2,3,3), stride=1, padding=0, optimizer="SGD")
    elif name == "1+padding":
        # 一个卷积核加padding
        c1 = layer.Conv2d((1,28,28), (1,3,3), stride=1, padding=2, optimizer="SGD")
    else:
        print("No model name found!")
        return None
    model.add_op(c1)
    model.add_op(layer.Relu())
    p2 = layer.Pool2d(c1.output_shape, (2,2), stride=2, padding=0, pool_type="max")
    model.add_op(p2)
    linear_shape = np.prod(p2.output_shape)
    print(linear_shape)
    model.add_op(layer.Flatten(p2.output_shape, (1, linear_shape)))
    model.add_op(layer.Linear(linear_shape, 2, init_method="kaiming", optimizer="Adam"))
...
def train_model(data_loader, model, model_name, show_loss=True):
    start = time.time()
    params = HyperParameters(max_epoch=50, batch_size=64)
