...
def build_model():
    ...
    c1 = layer.Conv2d((1,28,28), (6,5,5), stride=1, padding=2, optimizer="Adam")
    model.add_op(c1)
    model.add_op(layer.Relu())
    p1 = layer.Pool2d(c1.output_shape, (2,2), stride=2, padding=0, pool_type="max")
    model.add_op(p1)

    c2 = layer.Conv2d(p1.output_shape, (16,5,5), stride=1, padding=0, optimizer="Adam")
    model.add_op(c2)
    model.add_op(layer.Relu())
    ...
    model.add_op(layer.Linear(linear_shape, 120, init_method="kaiming", optimizer="Adam"))
    model.add_op(layer.BatchNorm1d(120))
    model.add_op(layer.Relu())
    model.add_op(layer.Linear(120, 84, init_method="kaiming", optimizer="Adam"))
    model.add_op(layer.Relu())
    model.add_op(layer.Linear(84, 26, init_method="kaiming", optimizer="Adam"))
...
def train_model(data_loader, model, model_name, show_loss=True):
    params = HyperParameters(max_epoch=10, batch_size=128)
    lrs = LRScheduler.step_lrs(0.01, 0.95, 500)
    training_history = train(data_loader, model, params, lrs, checkpoint=0.2)
    end = time.time()
    print("用时:", end-start)
