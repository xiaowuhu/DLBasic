...
def train_model(
    ...
    train_data_buf = np.ndarray(train_data_shape, dtype=np.float32, buffer=train_data_shm.buf)
    # 接收初始化权重参数
    event_update.wait()
    event_update.clear()
...
def main(model: Sequential, 
    ...
    for process_id in range(num_process):
        ...
        train_data_buf = np.ndarray(XY.shape, dtype=np.float32, buffer=train_data_shm.buf)
        event_data = Event()
        event_grad = Event()
        event_update = Event()
        event_data.clear()
    ...
    for epoch in range(params.max_epoch):
        ...
        for batch_id in range(batch_per_epoch):
            ...
            if iteration==1 or iteration % check_iteration == 0:
                stop = check_loss(data_loader,  params.batch_size, batch_id, model, training_history, epoch, iteration, params.learning_rate)
            if stop == True:
                break
        if stop == True:
            break
...
if __name__=="__main__":
    ...
    num_process = 4
    lrs = LRScheduler.step_lrs(0.01, 0.9, 100)
    model:Sequential = build_model()
    data_loader = load_minist_data()
    main(model, data_loader, params, num_process, lrs, checkpoint=0.1)
    test_model(data_loader, model, "MNIST_conv_14_7")
    #test_model(data_loader, model, "MNIST_conv_14_7_370")
    # test_model(data_loader, model, "MNIST_conv_14_7_900")
    # test_model(data_loader, model, "MNIST_conv_14_7_880")
    # test_model(data_loader, model, "MNIST_conv_14_7_820")
    # test_model(data_loader, model, "MNIST_conv_14_7_770")
    # test_model(data_loader, model, "MNIST_conv_14_7_690")
    # test_model(data_loader, model, "MNIST_conv_14_7_650")
    # test_model(data_loader, model, "MNIST_conv_14_7_420")
    # test_model(data_loader, model, "MNIST_conv_14_7_400")
