...
def build_model():
    ...
    c1 = layer.Conv2d((1,28,28), (8,5,5), stride=1, padding=0, optimizer="Adam")
    model.add_op(c1)
    ...
    model.add_op(layer.BatchNorm1d(linear_shape))
    model.add_op(layer.Linear(linear_shape, 10, init_method="kaiming", optimizer="Adam"))
...
def train_model(data_loader, model, model_name, show_loss=True):
    params = HyperParameters(max_epoch=10, batch_size=64, learning_rate=0.01)
