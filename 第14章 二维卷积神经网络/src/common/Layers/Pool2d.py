...
class Pool2d(Operator):
    ...
    def backward(self, dZ):
        if self.pool_type == "max":
            return self.backward_col2im_max(dZ)
        elif self.pool_type == "avg":
            return self.backward_col2im_avg(dZ)
        else:
    ...
    #     elif self.pool_type == "avg":
    #         dmax = np.repeat(dout.flatten() / self.kernel_size, self.kernel_size)
    ...
    def backward_col2im_max(self, dZ):
        dout = np.transpose(dZ, (0,2,3,1))
        dmax = np.zeros((dout.size, self.kernel_size)).astype(np.float32)
        dmax[np.arange(self.arg_max.size), self.arg_max.flatten()] = dout.flatten()
        dx = self.col2im(dmax, self.x.shape, self.kernel_height, self.kernel_width, self.stride, 0, self.output_height, self.output_width)
        return dx

    def backward_col2im_avg(self, dZ):
        dout = np.transpose(dZ, (0,2,3,1))
        davg = np.repeat(dout.flatten() / self.kernel_size, self.kernel_size)
        dx = self.col2im(davg, self.x.shape, self.kernel_height, self.kernel_width, self.stride, 0, self.output_height, self.output_width)
        return dx
