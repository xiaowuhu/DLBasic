...
class Conv2d(Operator):
    ...
    def calculate_padding_size(self, 
            ...
            stride=1):
        # 这段代码，当分母的计算结果不能被整除时，会向下取整，应该向上取整
        pad_h = ((output_height - 1) * stride - input_height + kernel_height) // 2
        pad_w = ((output_width - 1) * stride - input_width + kernel_width) // 2
        # pad_h = math.ceil(((output_height - 1) * stride - input_height + kernel_height) / 2)
        # pad_w = math.ceil(((output_width - 1) * stride - input_width + kernel_width) / 2)
        return (pad_h, pad_w)
