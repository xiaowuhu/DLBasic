...
def load_FashionMNIST_data():
    ...
    data_loader.load_FashionMNIST_data("image")
    data_loader.to_onehot(10)
    data_loader.StandardScaler_X(is_image=True)
    data_loader.shuffle_data()
    data_loader.split_data(0.96)
    return data_loader

def augment_data_loader():
    current_path = os.path.abspath(__file__)
    current_dir = os.path.dirname(current_path)
    train_filename = os.path.join(current_dir, "data", "FashionMNIST_Train.npz")
    test_filename = os.path.join(current_dir, "data", "FashionMNIST_Test.npz")
...
def build_model():
    ...
    c1 = layer.Conv2d((1,28,28), (16,3,3), stride=1, padding=1, optimizer="Adam")
    model.add_op(c1)
    model.add_op(layer.BatchNorm2d(16))
    model.add_op(layer.Relu())
    c2 = layer.Conv2d(c1.output_shape, (24,3,3), stride=1, padding=1, optimizer="Adam")
    model.add_op(c2)
    model.add_op(layer.BatchNorm2d(24))
    model.add_op(layer.Relu())
    s1 = layer.Pool2d(c2.output_shape, (2,2), stride=2, padding=0, pool_type="max")
    model.add_op(s1)

    c3 = layer.Conv2d(s1.output_shape, (32,3,3), stride=1, padding=1, optimizer="Adam")
    model.add_op(c3)
    model.add_op(layer.BatchNorm2d(32))
    model.add_op(layer.Relu())
    c4 = layer.Conv2d(c3.output_shape, (32,3,3), stride=1, padding=1, optimizer="Adam")
    model.add_op(c4)
    model.add_op(layer.BatchNorm2d(32))
    model.add_op(layer.Relu())
    c5 = layer.Conv2d(c4.output_shape, (24,3,3), stride=1, padding=1, optimizer="Adam")
    model.add_op(c5)
    model.add_op(layer.BatchNorm2d(24))
    model.add_op(layer.Relu())
    s2 = layer.Pool2d(c5.output_shape, (2,2), stride=2, padding=0, pool_type="max")
    model.add_op(s2)

    linear_shape = np.prod(s2.output_shape)
    model.add_op(layer.Flatten(s2.output_shape, (1, linear_shape)))
...
def train_model(data_loader, model, model_name, show_loss=True):
    ...
    lrs = LRScheduler.step_lrs(0.01, 0.95, 500)
...
if __name__=="__main__":
    model_name = "FahionMNIST_AlexNet_14_9"
    start = time.time()
    # 原始 Fashion-MNIST 数据
    # data_loader = load_FashionMNIST_data()
    # 图片增强后的数据
    data_loader = augment_data_loader()
    model = build_model()
