...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=conv_out, kernel_size=kernel, stride=1, padding=0),  # -> 20
        ...
        self.classifier = nn.Sequential(
            nn.BatchNorm1d(conv_out * w * w),
            nn.Linear(conv_out * w * w, 10), 
            # nn.Linear(conv_out * w * w, 128), 
            # nn.ReLU(inplace=True),
            # nn.Linear(128, 10), 
        )
    # 前向传递函数    
