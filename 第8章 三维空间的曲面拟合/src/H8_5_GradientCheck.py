...
if __name__=="__main__":
    ...
    epoch = 3000
    batch_size = 8
    params = HyperParameters(epoch, batch_size, lr)
    model = NN()
    print("权重矩阵初始化...")
    print(model.linear1.W)
    print(model.linear1.B)
    print(model.linear2.W)
    print(model.linear2.B)

    # 生成随机测试样本
    num_sample = 8
    X = np.random.rand(num_sample, 2)
    ...
    model.backward(Z, Y)
    # 自动微分法的梯度准备好了
    print(model.linear1.dW)
    print(model.linear1.dB)
    print(model.linear2.dW)
    print(model.linear2.dB)
    
    # 建立字典，生成矢量数据
    dict_param = {"W1": model.linear1.W, "B1": model.linear1.B, "W2": model.linear2.W, "B2": model.linear2.B}
    dict_grads = {"dW1": model.linear1.dW, "dB1": model.linear1.dB, "dW2": model.linear2.dW, "dB2": model.linear2.dB}
