...
def main(net:nn.Module, save_name:str, pretrained_model_name:str = None):
    ...
    #optimizer = torch.optim.SGD(model.parameters(), momentum=0.9, nesterov=True, lr=0.01, weight_decay=1e-6)
    optimizer = torch.optim.SGD(model.parameters(), momentum=0.9, lr=0.0001, weight_decay=5e-4)
    # 使用SGD做fine-tune
    ...
    best = 7250
    train_model(epoch, model, DEVICE, train_loader, test_loader, optimizer, exp_lr_scheduler, loss_func, save_name, best_correct=best)

if __name__=="__main__":
    start = time.time()    
    net = MobileNet()
    print(net)
    main(net, "MobileNet_Cifar100.pth", pretrained_model_name="MobileNet_Cifar100_7273.pth")
    #train_loader, test_loader = create_data_loader(64)
    #eval_model(net, test_loader, "MobileNet_Cifar100_7024.pth")
    end = time.time()
