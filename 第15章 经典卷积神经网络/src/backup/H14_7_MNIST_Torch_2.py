...
class ConvNet(nn.Module):
    def __init__(self):
        ...
        self.features = nn.Sequential(
            nn.Conv2d(in_channels=1, out_channels=8, kernel_size=3, stride=1, padding=0),  # -> 26
            nn.BatchNorm2d(8),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=8, out_channels=16, kernel_size=3, stride=1, padding=0),  # -> 24
            nn.BatchNorm2d(16),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # ->12

            nn.Conv2d(in_channels=16, out_channels=32, kernel_size=3, stride=1, padding=0),  # -> 24
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels=32, out_channels=64, kernel_size=3, stride=1, padding=0),  # -> 24
            nn.BatchNorm2d(64),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(kernel_size=2, stride=2),  # ->12
        )
...
def main():
    ...
    torch.save(ConvModel.state_dict(), "c:/Gitee/nnbasic/深度学习基础/第14章 二维卷积网络/src/model/ConvModel.pth")
