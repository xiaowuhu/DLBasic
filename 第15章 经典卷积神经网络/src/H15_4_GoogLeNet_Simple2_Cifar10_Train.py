...
def train_model(num_epochs, model, device, train_loader, test_loader, optimizer, lr_scheduler, loss_func, name):
    ...
    for epoch in range(num_epochs):
        ...
        for step, (train_x, train_y) in enumerate(train_loader):
            ...
            step_loss = step_loss1 * 0.3 + step_loss2
...
if __name__=="__main__":
    start = time.time()    
    net = GoogLeNet_Simple2(3, 10)
    #main(net, "GoogLeNet_Simple2_Cifar10.pth")
    train_loader, test_loader = create_data_loader(64)
    eval_model(net, test_loader, "GoogLeNet_Simple2_Cifar10_8321.pth")
    end = time.time()
