...
# 自定义的模型
class NN(Module):
    def __init__(self, init_method = "normal", optimizer = "SGD", regularizer = ("none", 0.0)):
    ...
    def forward(self, X, train=True):
        ...
        A = self.dropout.forward(A, train=train)
...
def check_loss(
        ...
        model: NN, 
        training_history:TrainingHistory_9, iteration:int):
    # 训练集
    x, y = data_loader.get_train()
    z = model.forward(x, train=False)
...
if __name__=="__main__":
    data_loader = load_data()
    params = HyperParameters(max_epoch=50000, batch_size=32, learning_rate=0.002)
    model = NN(init_method="xavier", optimizer="Momentum")
    training_history = train_model(data_loader, model, params, checkpoint=100)
    #model.save("Dropout_model")
    #training_history.save_history("training_history_9_7.txt")
