...
class Optimizer(object):
    ...
    def create_optimizer(optimizer):
        ...
        if name == "SGD":
            ...
            return RMSProp(param) if param is None else RMSProp(param)
        else:
...
class AdaGrad(Optimizer):
    def __init__(self):
        self.eps = 1e-6
        self.r = 0  # 记录历史梯度的平方和
        self.history = [] # debug only

    def update(self, lr, theta, grad):
        self.r = self.r + np.multiply(grad, grad)
        theta = theta - lr * grad / (np.sqrt(self.r) + self.eps)
        self.history.append(lr / (np.sqrt(self.r) + self.eps))
        return theta
