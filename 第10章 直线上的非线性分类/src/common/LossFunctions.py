...
class Logistic_CE2_loss(Operator):
    def forward(self, predict, label):  # 接收 linear 层输出的 z
        self.label = label
        # logistic
        self.predict = 1.0 / (1.0 + np.exp(-predict)) 
        # cross entropy 2
...
class Softmax_CE3_loss(Operator):
    def forward(self, predict, label):  # 接收 linear 层输出的 z
        self.label = label
        # softmax
        shift_z = predict - np.max(predict, axis=1, keepdims=True)
...
class my_loss(Operator):
    def forward(self, predict, label):
        p1 = label * (np.exp(1-predict)-1)
        p2 = (1-label) * (np.exp(predict)-1)
        return np.mean(p1 + p2)

    def backward(self, predict, label):
        return -label * np.exp(1-predict) + (1 - label) * np.exp(predict)
