...
def check_loss(
    ...
    #regular_loss = model.operator_seq[1].get_regular_cost()
...
def build_model():
    ...
    conv_stride = 1
    conv_padding = 0
    conv_output_length = 1 + (input_length + 2 * conv_padding - kernel_length) // conv_stride
    assert(conv_output_length > 1)

    pool_stride = 3
    pool_length = 3
    pool_padding = 0
    pool_output_length = 1 + (conv_output_length + 2 * pool_padding - pool_length) // pool_stride
    assert(pool_output_length > 0)

    model = Sequential(
        layer.Conv1d((input_channel, input_length), (output_channel, kernel_length), 
                      stride=conv_stride, padding=conv_padding, init_method="normal", optimizer="Adam"),
        layer.Pool1d((output_channel, conv_output_length), pool_length, pool_stride),
        layer.Flatten((output_channel, pool_output_length), pool_output_length * output_channel),
        layer.Linear(pool_output_length * output_channel, 4, regularizer=("L1", 0.5), init_method="normal", optimizer="Adam"),
    )
...
if __name__=="__main__":
    ...
    params = HyperParameters(max_epoch=20, batch_size=32, learning_rate=0.01)
    lrs = LRScheduler.step_lrs(0.01, 0.9, 1000)
    training_history = train_model(data_loader, model, params, lrs, checkpoint=1)
    training_history.show_loss()
    #model.save("model_13_7_conv_pool2")
    # # test
