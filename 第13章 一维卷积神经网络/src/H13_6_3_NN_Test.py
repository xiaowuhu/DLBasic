...
def test_model(data_loader: DataLoader_13, model: Sequential):
    ...
    #         ax3.plot((0, x),(0, y), marker='.',label=str(i+1))
    #         ax3.text(x, y, "$c_"+str(i+1)+"$")
    #         x, y = W[4,i], W[4,i]
    #         ax3.plot((0,x),(0,y), marker='^', label=str(i+1))
    #         ax3.text(x, y, "$w_"+str(i+1)+"$")

    # ax1.grid()
    # ax1.set_title("类别:2,6")
    # ax2.grid()
    # ax2.set_title("类别:4,8")
    # ax3.set_title("类别:1,3,5,7")
    # ax3.grid()
    # plt.show()
...
def show_filter_shape(model):
    ...
    print(filter)
    titles = ["$w_1$:2,6","$w_2$:4,8","$w_3$:1,3,5,7"]
...
if __name__=="__main__":
    ...
    model.load("model_13_6_conv_out3")
