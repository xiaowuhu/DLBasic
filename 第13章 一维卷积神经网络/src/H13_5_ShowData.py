...
def show_class_data_1(class_id, ax):
    for i in range(X.shape[0]):
        if Y[i] == class_id:
            x = np.linspace(0,1,X[i].shape[0])[0:-2]
            ax.plot(x, X[i,0:-2], marker='.', label=str(class_id+1))
...
def show_class_data_2(class_id, ax, label):
    for i in range(X.shape[0]):
        if Y[i] == class_id:
            x = np.linspace(0,1,X[i].shape[0])[2:]
            ax.set_title(label)
            ax.plot(x, X[i,2:], marker='.', label=str(class_id+9))
            ax.legend()
            break
